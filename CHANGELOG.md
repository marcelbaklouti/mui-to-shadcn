...
- **TextField `slotProps`/`inputProps`/`inputRef`/`size` no longer leak.** The v6/v9 `slotProps` (and v5 `inputProps`/`inputRef`/`size`) were passed through onto the shadcn `Input` as invalid props (and adornment imports could dangle). They are now consumed with a warning pointing at input-group for adornments and the native attributes for `htmlInput`.
...
- **Named / `useCallback` handlers are rewritten, not just renamed.** Previously only inline-arrow handlers were fixed; a handler passed by reference (`onChange={handleCheck}`, or a `useCallback`-wrapped one) was renamed to `onCheckedChange`/`onValueChange` while its body still read `e.target.checked`/`e.target.value` — a runtime `TypeError` on first interaction. The handler's same-file definition is now resolved and rewritten with the same safety guard (only when the event param is used *solely* as `e.target.value`/`checked`); a handler that also uses the event (e.g. `e.preventDefault()`) is left untouched with the existing warning.
